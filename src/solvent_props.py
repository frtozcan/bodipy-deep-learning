...
KAMLET-TAFT (pi*, alpha, beta): TAMAMI Kamlet, Abboud, Abraham & Taft,
J. Org. Chem. 1983, 48, 2877-2887, Tablo I'den DOGRULANMISTIR (makale ref [23]).
Kaynakta parantezli verilen (dogrudan olculmemis, tahmin edilmis) degerler asagida
'~' ile isaretlidir: DCM alpha, MeOH beta, EtOH beta, BuOH beta, 2-PrOH beta, su beta.
Kamlet 1983 Tablo I'de BULUNMAYAN cozuculer (1-oktanol, 2-MeTHF) icin Kamlet-Taft
degerleri None birakilmistir -> egitim medyani ile doldurulur + eksik gostergesi.
Pinakolon = "methyl tert-butyl ketone" (Tablo I no. 65): yalnizca beta verilmis.

ET(30): TAMAMI Reichardt, Chem. Rev. 1994, 94, 2319-2358, Tablo 2'den DOGRULANMISTIR
(makale ref [22]). 22 cozucunun 20'si birebir tutti; 1-oktanol 48.3 -> 48.1 duzeltildi,
pinakolon (no. 240, "3,3-dimethyl-2-butanone") 39.0 olarak eklendi. 2-MeTHF (no. 216)
Reichardt'ta VAR (36.5) ve dogrulandi, ancak Kamlet-Taft tablosunda yok.

eps_r / n_D / eta (viskozite): standart derlemelerden; BUNLAR HENUZ BIRINCIL KAYNAKTAN
DOGRULANMADI. Kamlet-Taft ve ET(30) dogrulamasi tamamlandi.
...
SOLVENTS = {
    ...
    'ClC(Cl)Cl':        ('chloroform',       4.81, 1.4459, 39.1, 0.537,  0.58, 0.44, 0.00),
    ...
    'CC(C)=O':          ('acetone',         20.70, 1.3588, 42.2, 0.306,  0.71, 0.08, 0.48),
    'OCC(F)(F)F':       ('TFE',             26.67, 1.2907, 59.8, None,   0.73, 1.51, 0.00),
    'C1COCCO1':         ('1,4-dioxane',      2.21, 1.4224, 36.0, 1.177,  0.55, 0.00, 0.37),
    'CC(=O)C(C)(C)C':   ('pinacolone',       None, None,   39.0, None,   None, None, 0.48),
    'O':                ('water',           78.36, 1.3330, 63.1, 0.890,  1.09, 1.17, 0.18),  # beta ~
    'CCOCC':            ('diethyl ether',    4.27, 1.3524, 34.5, 0.224,  0.27, 0.00, 0.47),
    'CCCCCCCCO':        ('1-octanol',       10.30, 1.4295, 48.1, 7.288,  None, None, None),  # KT: Kamlet'te yok
    'CCCCO':            ('1-butanol',       17.51, 1.3993, 49.7, 2.544,  0.47, 0.79, 0.88),  # beta ~
    'CC1CCCO1':         ('2-MeTHF',          6.97, 1.4059, 36.5, None,   None, None, None),  # Kamlet'te yok
    'CC(C)O':           ('2-propanol',      19.92, 1.3776, 48.4, 2.038,  0.48, 0.76, 0.95),  # beta ~
}
